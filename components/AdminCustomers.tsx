...
import { importWooCommerceData, subscribeToCustomers, CustomerProfile } from '../services/firestoreService';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
        ...
        if (typeof text === 'string') {
            setStatusMsg("Importing data to Firestore...");
            try {
                const result = await importWooCommerceData(text);
                setStatusMsg(`Success! Imported ${result.success} records. (${result.errors} skipped)`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-3">
            <button 
                ...
                className="bg-navy-900 dark:bg-white dark:text-navy-900 text-white font-bold px-4 py-2 rounded-lg transition-colors shadow-lg disabled:opacity-50 flex items-center gap-2 text-xs"
            >
                {isUploading ? (
